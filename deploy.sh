...
fs.suid_dumpable=0
abi.vsyscall32=0
" >/lib/sysctl.d/99-swappiness.conf
...
# Install boot loader
if [ "$ENCRYPTED" = "y" ]; then
  sed -i -e "s|GRUB_CMDLINE_LINUX_DEFAULT=.*|GRUB_CMDLINE_LINUX_DEFAULT=\"cryptdevice=$PART2:root:allow-discards root=/dev/mapper/root quiet rootfstype=ext4,btrfs,xfs,f2fs biosdevname=0 nowatchdog noautogroup noresume cryptomgr.notests\"|g" /etc/default/grub
